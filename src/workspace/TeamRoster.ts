...
export async function generateTeamRoster(config: RootConfig): Promise<void> {
  ...
  lines.push('')
  lines.push('#### 定时进度汇报（所有角色）')
  lines.push('')
  lines.push('**任何任务超过 2 分钟未完成，必须主动在飞书会话中发送进度更新，之后每 2 分钟汇报一次，直至完成。**')
  lines.push('')
  lines.push('汇报格式：')
  lines.push('```')
  lines.push('【进度更新】<任务/计划名称>')
  lines.push('当前状态：<正在做什么>')
  lines.push('已完成：<已完成的部分>')
  lines.push('剩余：<还需要做什么>')
  lines.push('预计：<预计还需多久>')
  lines.push('```')
  lines.push('')
  lines.push('### 共享产出')
  lines.push('')
